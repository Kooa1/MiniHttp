...
一个基于 **Linux epoll** 的 C++17 HTTP 服务器，采用 **主从 Reactor（Master-Slave Multi-Reactor）** 架构。内置线程池处理 CPU 密集型任务，中间件管道支持洋葱模型请求链，静态文件服务基于 sendfile 零拷贝。具备连接保活（HTTP/1.1 keep-alive）、连接超时管理、跨线程安全关闭等生产级特性。在 2 vCPU 云服务器上可达 **43,643 RPS** 吞吐量。
...
| 场景 | 并发 | RPS | 传输吞吐 | 平均延迟 | 最大延迟 | p50 | p99 |
|------|------|-----|---------|---------|---------|-----|-----|
...
| GET static/index.html | 1 | 9,229 | 27.42MB | 50.44ms | 1.08s | 81.00us | 967.21ms |
| GET static/index.html | 10 | 25,538 | 75.87MB | 164.66ms | 1.57s | 262.00us | 1.43s |
| GET static/index.html | 50 | 24,730 | 73.47MB | 170.40ms | 1.50s | 1.28ms | 1.43s |
| GET static/index.html | 100 | 25,585 | 76.01MB | 116.48ms | 1.22s | 2.88ms | 1.10s |
| GET static/style.css | 1 | 10,540 | 864.57KB | 3.73ms | 273.81ms | 78.00us | 165.99ms |
| GET static/style.css | 10 | 13,516 | 1.08MB | 96.92ms | 1.51s | 246.00us | 1.39s |
| GET static/style.css | 50 | 16,404 | 1.31MB | 1.33ms | 22.54ms | 1.20ms | 5.01ms |
| GET static/style.css | 100 | 14,144 | 1.13MB | 242.68ms | 1.95s | 3.12ms | 1.81s |
...
- **sendfile 效率**: 大文件（`/static/index.html`）传输吞吐达 **76 MB/s**，RPS 约 25,000；小文件（`/static/style.css`）在并发 50 时 p99 仅 **5.01ms**
